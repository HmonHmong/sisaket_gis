...
<div class="max-w-[1600px] mx-auto pb-20 animate-in fade-in duration-700 px-4">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
        ...
        <form method="GET" class="flex flex-wrap gap-3 w-full lg:w-auto bg-white p-3 rounded-[2rem] shadow-sm border border-slate-100">
            <select name="user_id" class="p-2.5 bg-slate-50 border-none rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500 text-slate-700">
            ...
            <select name="action" class="p-2.5 bg-slate-50 border-none rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500 text-slate-700">
                ...
                <option value="MAINTENANCE" <?= $f_action == 'MAINTENANCE' ? 'selected' : '' ?>>บำรุงรักษาระบบ</option>
                <option value="BACKUP" <?= $f_action == 'BACKUP' ? 'selected' : '' ?>>สำรองข้อมูล</option>
            </select>
                ...
                <tbody class="divide-y divide-slate-50">
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): 
                            $action_colors = [
                                'INSERT' => 'bg-emerald-50 text-emerald-600 border border-emerald-100',
                                'UPDATE' => 'bg-blue-50 text-blue-600 border border-blue-100',
                                'DELETE' => 'bg-rose-50 text-rose-600 border border-rose-100',
                                'LOGIN' => 'bg-orange-50 text-orange-600 border border-orange-100',
                                'MAINTENANCE' => 'bg-purple-50 text-purple-600 border border-purple-100',
                                'BACKUP' => 'bg-teal-50 text-teal-600 border border-teal-100'
                            ];
                            $a_color = $action_colors[$row['action']] ?? 'bg-slate-50 text-slate-600 border border-slate-200';
                        ?>
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="py-6 px-4">
                                <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest border border-slate-100 bg-slate-50 px-3 py-1 rounded-lg shadow-sm">
                                    <?= e($row['target_type'] ?: 'ระบบ') ?>
                                </span>
                            </td>
                            <td class="py-6 px-4">
                            ...
                            <td class="py-6 px-10 text-right">
                                <span class="text-[10px] font-mono font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded"><?= $row['ip_address'] ?></span>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="py-20 text-center text-slate-300">
                                <i data-lucide="search-x" class="mx-auto mb-3 opacity-50" size="40"></i>
                                <p class="italic text-sm font-bold">ไม่พบประวัติกิจกรรมในช่วงเวลานี้</p>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
